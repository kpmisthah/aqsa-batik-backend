...
export const sendOtpEmail = async (email: string, otp: string, name: string): Promise<string | null> => {
  ...
  try {
    const info = await t.sendMail({
      from: `"Aqsha Batik Store" <${process.env.SMTP_USER}>`,
      to: email,
      subject,
      html: htmlContent,
    });
...
/**
 * Send a Password Reset OTP code to a recipient email address
 */
export const sendResetPasswordOtpEmail = async (email: string, otp: string, name: string): Promise<string | null> => {
  const t = await initTransporter();

  const subject = '🔒 Aqsha Batik Store - Reset Your Password';
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #5A2A1F/10; border-radius: 12px; background-color: #FDFBF7;">
      <div style="text-align: center; margin-bottom: 20px;">
        <span style="font-size: 10px; font-weight: bold; letter-spacing: 2px; color: #8B3A2B; text-transform: uppercase;">AQSHA STORE</span>
        <h2 style="color: #5A2A1F; font-family: Georgia, serif; margin: 10px 0 0 0;">Reset Your Password</h2>
      </div>
      <p style="color: #5A2A1F; font-size: 14px; line-height: 1.5;">Hi <strong>${name}</strong>,</p>
      <p style="color: #5A2A1F; font-size: 14px; line-height: 1.5;">We received a request to reset your password for your Aqsha Batik Store account. Please use the following One-Time Password (OTP) to complete the verification process. This code is valid for 10 minutes:</p>
      <div style="text-align: center; margin: 30px 0;">
        <div style="display: inline-block; padding: 15px 40px; background-color: #5A2A1F; color: #ffffff; font-size: 32px; font-weight: bold; letter-spacing: 5px; border-radius: 8px; font-family: monospace; box-shadow: 0 4px 6px rgba(90,42,31,0.15);">${otp}</div>
      </div>
      <p style="color: #5A2A1F; font-size: 12px; line-height: 1.5; color: #5A2A1F/60;">If you didn't request a password reset, please ignore this email or contact support if you have concerns.</p>
      <hr style="border: 0; border-top: 1px solid rgba(90,42,31,0.1); margin: 30px 0 10px 0;">
      <p style="text-align: center; font-size: 10px; color: #5A2A1F/40; margin: 0;">© ${new Date().getFullYear()} Aqsha Batik Cloth. All rights reserved.</p>
    </div>
  `;
  ...
  // Log to console so developer always sees the OTP immediately in the terminal
  console.log(`\n🔑 [RESET PASSWORD OTP CODE] Sending OTP to ${email}: ${otp} (User: ${name})\n`);
  ...
  try {
    ...
    const previewUrl = nodemailer.getTestMessageUrl(info);
    if (previewUrl) {
      console.log(`✉️ Password reset email sent! View preview: ${previewUrl}`);
      return previewUrl;
    }
